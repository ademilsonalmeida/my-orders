...
export default function PerfilPage() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (userProfile) {
          setUserProfile(userProfile.session);
          form.reset({
            whatsapp: userProfile.profile.whatsapp
          });
        }
      } catch (error) {
  ...
  async function onSubmit(values: z.infer<typeof profileSchema>) {
    try {
      await updateProfile(values.whatsapp);
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Aqui estão todos os seus dados.</p>
            </div>            
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="destructive" type="button" disabled={form.formState.isSubmitting || isLoading}>
                  {form.formState.isSubmitting && (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  )}
                  Remover minha conta
